...
* Playing to the strenghts of the platform
  ...
  in our PR workflow.]. Instead we run =tox= directly from the main
  container of our =test_lp{eu,usa}= jobs to install all dependencies.
  We launch auxiliary services using CircleCI's native [[https://circleci.com/docs/2.0/configuration-reference/#docker--machine--macosexecutor][service
  container support]] (rather than =docker-compose=), and run tests
  directly with =tox=. This avoids minutes of saving the image to---and
  restoring it from---our workspace. We also avoided the extra
