...
* Playing to the strenghts of the platform
  ...
  We didn't want to change the way developers ran tests locally. But
  we moved to use the lower-level Tox API for running tests in our CI
  job.

  First we changed the CI test workflow to no longer depend on
  building the base image[fn::We still build it, because we want to
  test that we can, since we use it in our deploy workflow still. But
  we no longer need to save it to disk or persist it to the workspace
  in our PR workflow.]. Instead we install all dependencies into the
  main container of the CircleCI job. We launch auxiliary services
  using CircleCI's native [[https://circleci.com/docs/2.0/configuration-reference/#docker--machine--macosexecutor][service container support]] (rather than
  =docker-compose=), and run tests directly with Tox. This avoided
  minutes of saving the image to---and restoring it from---our
  workspace. We also avoided the extra startup-cost of the =machine=
  executor.
  ...
  settled on running the tests in parallel across 3 CPUs[fn::By default
