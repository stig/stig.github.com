...
out-of-the-box thinking we managed to get the full PR cycle for this
...
configuration, for each skipping only the tests we know are not
applicable for that profile[fn::About 10% of the total.].
...
of build output to find out.
Finally, we had to wait until the last had run to find out if they
were all successful.

We split it into separate jobs they can run concurrently, addressing
all these shortcomings. Doing so we realised that none of them needed
to wait for the =build= job to finish. Picture [[april]] shows the resulting
workflow. The /total/ time has not improved[fn::It's a little improved,
but I think that was down to being lucky with cache when installing
dependencies.], but we get more useful information much sooner. The
slowest of the new jobs now finish after about two minutes, before the
build job even finishes.
...
duplication---by building & saving a base docker image we would later
use to run the tests in the =build= job, as well as /restoring/ this
docker image in the two =test_lp{eu,usa}= jobs---was more time consuming
than we had expected:

- We use [[https://circleci.com/docs/2.0/docker-layer-caching/][Docker Layer Caching (DLC)]] on PR branches[fn::On the main
  branch we disable the DLC, so building the image takes a bit longer
  here.], so if your PR didn't change runtime dependencies building
  the docker image was usually pretty fast[fn::In our /deploy/ workflow
  (not discussed in this post) we deploy this image on
  ElasticBeanstalk, so we omitted test dependencies.].

- However, a =docker save= to a file on disk took around 30 seconds, and
  because the resulting file is rather big [[https://circleci.com/docs/2.0/configuration-reference/#persist_to_workspace][persisting it to the
...
- These test jobs also mimiced the way most local devs work, manually
  started up dependencies (Redis, Cassandra, and PostgreSQL) with
  =docker-compose=. This required us to use the [[https://circleci.com/docs/2.0/executor-types/][machine executor]], which
  added about 30-60 second startup overhead compared to the docker
  executors.
